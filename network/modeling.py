...
def _segm_efficientnet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    backbone = efficientnet_lite.build_efficientnet_lite("efficientnet_lite0", 21)
    # print(backbone)
    # rename layers
    backbone.low_level_features = backbone.features
    backbone.high_level_features = backbone.blocks  # [1:]
    backbone.features = None
...
def deeplabv3plus_efficientnet(num_classes=21, output_stride=8, pretrained_backbone=True):
    """Constructs a DeepLabV3+ model with a MobileNetv2 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3plus', 'efficientnet', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone)